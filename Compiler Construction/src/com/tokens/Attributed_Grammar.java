...
public class Attributed_Grammar {
	...
	public static void main(String[] args) throws Exception {
		...
		new Attributed_Grammar();
		
//		database.deleteErntries();
	}
	
	public Attributed_Grammar() throws Exception {
...
//				}
		...
		if(tokenSet.get(i).get(0).equals ("DT")) {
			...
			if(tokenSet.get(i).get(0).equals ("ID")) {
				...
				if(tokenSet.get(i).get(0).equals ("ID")) {
					...
					if(init2(name,type,TM,AM,CR,isStatic)) {
	...
	private boolean CST4_dash(String name,String type,String AM,int isStatic,int CR) {
		...
		if(tokenSet.get(i).get(0).equals ("=") || tokenSet.get(i).get(0).equals (";") ||
			...
			System.out.println("here->"+name+" line no: "+tokenSet.get(i).get(2));
	...
	private boolean case_body(String type,int CR,int FR) {
		...
		if(tokenSet.get(i).get(0).equals ("const")) {
			...
			if(tokenSet.get(i).get(0).equals ("ID")) {
				...
				}else if(!database.insert_FT(FR, name, type1, TM)) {
					System.out.println("Redeclaration Error,line #="+tokenSet.get(i).get(2));
				}
	...
	private boolean case_dash(String TM,int CR,int FR) {
		...
		if(tokenSet.get(i).get(0).equals ("DT")) {
			...
			if(tokenSet.get(i).get(0).equals ("ID")) {
				...
				if(T!=null) {
					System.out.println("Already define in class,line #="+tokenSet.get(i).get(2));
				}else if (!database.insert_FT(FR, name, type, TM)) {
					System.out.println("Redeclaration Error,line #="+tokenSet.get(i).get(2));
				}
			...
			if(tokenSet.get(i).get(0).equals ("ID")) {
				...
				if(init2(name,type,TM,"Default",FR,0)) {
	...
	private boolean case2(String type,int CR,int FR) {
		...
		if(tokenSet.get(i).get(0).equals(",") || tokenSet.get(i).get(0).equals("=") ||
			...
			if(init2(type,name,"null","Default",0,FR)) {
...
//			switch, ID, this, base, inc_dec, return, pass, break, try, String, var, throw }
	...
	private boolean SST_dash(String TM,int CR,int FR) {
		...
		if(tokenSet.get(i).get(0).equals("DT")) {
			...
			if(tokenSet.get(i).get(0).equals("ID")) {
				...
				if(init2(name,type,TM,"Defalut",0,FR)) {
	...
	private boolean init2(String name,String type,String TM,String AM,int isStatic,int id) {
		String name2="";
		if(tokenSet.get(i).get(0).equals("=")) {
			i++;
...
//				sealed, while, for, if, do, forEach, switch, this, base, inc_dec, return, pass, break, try, throw, elif, else, } }
	...
	private boolean c3_dash(String type) {
		if(ass_opr()) {
			String op=Op;
			Op="null";
			if(OE()) {
	...
	private boolean RST() {
		...
		if(tokenSet.get(i).get(0).equals(";")) {
			...
			if(F()) {
				...
				type = database.compatibility(type);
				if(type == null) {
		...
		if(tokenSet.get(i).get(0).equals("const") || tokenSet.get(i).get(0).equals("DT")) {
			...
			if(tokenSet.get(i).get(0).equals("DT")) {
				...
				if(tokenSet.get(i).get(0).equals("ID")) {
					...
					}else if(!database.insert_FT(FR, name, type, "null")){
						System.out.println("Redeclaration Error");
					}
...
//			if(tokenSet.get(i).get(0).equals("int") || tokenSet.get(i).get(0).equals("float") 
//					|| tokenSet.get(i).get(0).equals("short") || tokenSet.get(i).get(0).equals("long")
//					|| tokenSet.get(i).get(0).equals("char") || tokenSet.get(i).get(0).equals("bool")) {
//				
//				i++;
//				return true;
//			}
			...
			if(OE()) {
				...
				if (T==null) {
					...
					System.out.println("Type Error in init");
				}
	...
	private boolean OE_dash() {
		...
		if(tokenSet.get(i).get(0).equals("OR")) {
			...
			if(AE()) {
				...
				type = database.compatibility(LO, RO, Op);
				if(type == null) {
